...
(async () => {
  ...
  try {
    ...
    const open = async query => { await page.goto('http://localhost:1421/tests/fixtures/agent-backups.html?' + query); await page.getByRole('button', { name: '手动备份', exact: true }).waitFor(); };
    const backup = async () => { await page.getByRole('button', { name: '手动备份', exact: true }).click(); await page.getByText('已手动备份当前磁盘配置，未包含未保存的表单修改。', { exact: true }).waitFor(); };
    const choose = async () => { await page.getByRole('button', { name: '恢复备份', exact: true }).click(); await page.locator('.agent-backup-columns nav button').first().click(); };
    for (const query of ['fresh', 'embedded&fresh', 'client=pi']) {
      await open(query);
      if (!query.includes('pi')) {
      ...
      await backup();
      assert.deepEqual((await calls('create_agent_config_backup'))[0].args, { client: query.includes('pi') ? 'pi' : 'codex' });
    ...
    await page.screenshot({ path: 'misc/agent-backups-narrow.png', fullPage: true });
    await open('client=claude-desktop');
    await page.getByText('没有可靠的 Claude Desktop 模型映射，请重新选择模型。', { exact: true }).waitFor();
    assert.ok(await page.getByRole('button', { name: '更新配置', exact: true }).isDisabled());
    assert.deepEqual(errors, []);
    console.log('PASS: full/compact/Pi backup, disk-only payload, template preview, restore/delete confirmations, corrupt backups, conflict invalidation, narrow layout, missing Desktop mapping');

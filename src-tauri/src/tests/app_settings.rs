...
#[test]
fn api_balance_validation_returns_stable_error_codes() {
    let invalid_url = GuiApiBalanceEndpoint {
        provider_section: "openai-compatibility".to_string(),
        record_identity: "0".repeat(64),
        balance_url: "not-a-url".to_string(),
    };
    assert_eq!(
        validate_gui_api_balance_endpoint(&invalid_url),
        Err(API_BALANCE_INVALID_URL.to_string())
    );

    let insecure_url = GuiApiBalanceEndpoint {
        balance_url: "http://example.com/balance".to_string(),
        ..invalid_url
    };
    assert_eq!(
        validate_gui_api_balance_endpoint(&insecure_url),
        Err(API_BALANCE_INSECURE_URL.to_string())
    );
    assert_eq!(
        api_access_record_identity("invalid", "name", "https://example.com", &[]),
        Err(API_BALANCE_INVALID_PROVIDER.to_string())
    );
}

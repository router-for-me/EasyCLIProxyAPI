...
const apiQuotaBackendErrorKeys: Partial<Record<string, MessageKey>> = {
  'api_balance.invalid_provider': 'quota.api.error.invalidProvider',
  'api_balance.invalid_identity': 'quota.api.error.invalidIdentity',
  'api_balance.invalid_url': 'quota.api.error.invalidUrl',
  'api_balance.insecure_url': 'quota.api.error.insecureUrl',
  'api_balance.invalid_record_name': 'quota.api.error.invalidRecordName',
...
export const resolveApiAccessBalanceUrls = async (
  ...
  try {
    return await invoke<Array<string | null>>('resolve_api_access_balance_urls', { queries });
  } catch (error) {
    throw new Error(apiQuotaErrorMessage(error));
  }
};
